...
def get_node_classification_data(dataset_name: str, val_ratio: float, test_ratio: float):
    ...
    # get the timestamp of validate and test set
    print("ts min:", graph_df['ts'].min())
    print("ts max:", graph_df['ts'].max())
    print("ts quantiles:", np.quantile(graph_df['ts'], [0.6, 0.9]))
    print("train size:", sum(graph_df['ts'] <= np.quantile(graph_df['ts'], 0.6)))
